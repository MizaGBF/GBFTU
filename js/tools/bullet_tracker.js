class BulletTracker extends ToolBase
{
	...
	static c_bullets = Object.freeze({
		...
		// FMJ
		"20101":[{}, {"34":3,"1169":3,"9":7,"1032":2}, {}],
		"20102":[{"20101":1}, {"34":7,"9":10,"1032":5}, {}],
		"20103":[{"20102":2}, {"34":12,"71":10,"5":12}, {}],
		"20104":[{"20103":5}, {"34":25,"71":20,"1169":25}, {}],
		"20105":[{"20104":5}, {"34":30,"71":25,"1169":30}, {}],
		// exploder
	...
	update()
	{
		...
		if(has_content)
		{
			...
			for(const [bullet, count] of Object.entries(this.data))
			{
				let container = add_to(span, "span");
			...
			// dependant bullets
			span = add_to(null, "span");
			for(const [bullet, count] of Object.entries(dependancies))
			{
				let final_count = count - (bullet in this.data ? this.data[bullet] : 0);
				if(final_count > 0)
				{
					let container = add_to(span, "span");
					container.style.display= "inline-block";
			...
			for(const [id, count] of Object.entries(loot))
			{
				...
				container.style.width = "max-content";
				container.style.marginRight = "5px";
				add_to(container, "img", {cls:["tool-icon"]}).src = "https://prd-game-a-granbluefantasy.akamaized.net/assets_en/img_low/sp/assets/item/article/s/" + id + ".jpg";
				container.appendChild(document.createTextNode(" x" + count));
			}
			for(const [id, count] of Object.entries(evolution))
			{
				let container = add_to(span, "span");
				container.style.display= "inline-block";
				container.style.width = "max-content";
